...
class BaseFileUploader(object):
    ...
    async def add_title_tags(self, page):
        ...
        """
        try:
            # 等待标题/正文输入框出现(抖音发布页组件异步渲染,直接 find_button 可能为 0)
            if self.title_supported and self.title:
                for sel in self.editor_button_locators:
                    try:
                        await page.wait_for_selector(sel, state='attached', timeout=20000)
                        break
                    except Exception:
                        continue
            if self.textbox_supported and self.text:
                for sel in self.textbox_selectors:
                    try:
                        ...
                        continue
            # 输入标题(标题是 input 元素,用 fill 最可靠,能正确触发受控组件 change)
            if self.title_supported and self.title:
                ...
                # fill 直接设置 value(自动触发 input 事件,避免 insert_text 中文乱码)
                try:
                    await editor_button.fill(self.title)
                except Exception as fill_err:
                    self.logger.warning(f"标题 fill 失败,回退键盘输入: {fill_err}")
                    await page.keyboard.press("Control+A")
                    await page.keyboard.press("Delete")
                    await page.keyboard.type(self.title, delay=50)
                await page.wait_for_timeout(self.wait_timeout_500ms)

            # 输入正文(contenteditable,fill 不适用,用模拟键盘 type 逐个字符输入)
            if self.textbox_supported and self.text:
                textbox_button = await self.find_button(self.textbox_selectors)
                if not textbox_button:
                ...
                await page.keyboard.press("Delete")
                await page.wait_for_timeout(self.wait_timeout_500ms)
                # 用 type 模拟键盘逐个输入(触发 input 事件,中文更可靠)
                await page.keyboard.type(self.text, delay=50)
                await page.wait_for_timeout(self.wait_timeout_500ms)
    ...
    async def click_publish(self, page):
        ...
        while attempt < max_attempts and not self.publish_status:
            ...
            try:
                ...
                # 步骤1.5: 检测安全验证(手机验证码/滑块)弹窗——抖音等平台发布前可能要求人机验证。
                # 系统无法自动绕过短信验证码,这里保持浏览器打开,等待用户在窗口中手动输入后继续。
                try:
                    page_content = await page.content()
                    if any(k in page_content for k in ['安全验证', '短信验证', '验证码', '请输入验证码', '滑块验证']):
                        self.logger.warning("检测到安全验证弹窗,请在浏览器窗口中手动完成验证(等待最长180秒)...")
                        for _ in range(180):
                            await asyncio.sleep(1)
                            try:
                                cur = await page.content()
                                # 弹窗消失(用户完成验证)或离开发布页则继续
                                if not any(k in cur for k in ['安全验证', '短信验证', '滑块验证']):
                                    break
                                if page.url != current_url and 'post' not in page.url:
                                    break
                            except Exception:
                                break
                        self.logger.info("安全验证等待结束,继续发布流程")
                except Exception:
                    pass
