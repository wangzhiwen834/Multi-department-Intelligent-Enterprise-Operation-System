import type { Shop, Template, Workbook, WorkbookListItem, BootstrapPayload, LockStatus, ExtractResult, User, DashboardOverview, AuditLogPage, Logo, AiSettings, AiModelKind, Business, HotelOverview, TiaoliOverview } from './types';
...
export const api = {
  ...
  dashboardOverview: (granularity: 'day' | 'week' | 'month' | 'year', date: string, shopId?: number, businessCode = 'footbath') =>
    req<DashboardOverview | HotelOverview | TiaoliOverview>(`/api/dashboard/overview?granularity=${granularity}&date=${date}${shopId ? `&shopId=${shopId}` : ''}&businessCode=${businessCode}`),
