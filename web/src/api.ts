...
export const api = {
  ...
  createShop: (name: string, businessId?: number) =>
    req<Shop>('/api/shops', { method: 'POST', body: JSON.stringify({ name, businessId }) }),
  ...
  dashboardOverview: (granularity: 'day' | 'week' | 'month' | 'year', date: string, shopId?: number, businessCode = 'footbath') =>
    req<DashboardOverview | HotelOverview | TiaoliOverview | YueziOverview>(`/api/dashboard/overview?granularity=${granularity}&date=${date}${shopId ? `&shopId=${shopId}` : ''}&businessCode=${businessCode}`),
  ...
  // SSE 登录流
  mediaLoginStream: (type: number, id: string, onMessage: (data: any) => void): EventSource => {
    const token = getToken();
    const url = `/api/media/login/stream?type=${type}&id=${encodeURIComponent(id)}`;
    const es = new EventSource(url, { withCredentials: false });
    // 由于 EventSource 不能自定义 header，我们通过 URL 传 token 不太安全
    // 改用: 后端通过 cookie 或 query 传 token
    // 简单起见，暂时通过 fetch 方式处理（下面有另一个实现）
    return es;
  },
