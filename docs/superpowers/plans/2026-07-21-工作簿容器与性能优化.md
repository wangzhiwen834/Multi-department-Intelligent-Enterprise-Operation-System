...
- Modify: `server/src/workbook/workbook.routes.ts`（加 `activeOnlySnapshot` + 2 路由）
...
- [ ] **Step 5: 加 activeOnlySnapshot 辅助 + 2 路由**
...
Run: `cd server && npm run test -- workbook-bootstrap`
Expected: PASS（4 个用例全过）。再跑全量 `cd server && npm run test` 确认无回归。
Expected: 全部 PASS。
...
git commit -m "feat: bootstrap 引导加载+按表取 cellData+gzip 压缩"
```
