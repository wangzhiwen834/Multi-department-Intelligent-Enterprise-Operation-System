...
- [ ] **Step 1: 替换 `<script setup>` 与 `<template>` 两块(保留文件现有 `<style scoped>` 不动)**

把 `web/src/views/ShopList.vue` 的 `<script setup>...</script>` 与 `<template>...</template>` 两个块整体替换为下面的内容(`<style>` 块保留原样,不动):
